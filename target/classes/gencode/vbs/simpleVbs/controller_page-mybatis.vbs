...
Function init()
   ...
   If signLen < 1 Then
		...
		output "controller的classPage:" + classPage
...
Function CreateEntityPageFolder(vfso, vpath, vpage)
	...
	For Each temp in tempArray
	  ...
	  Output "创建controller目录：" + CreateEntityPageFolder
    Next
...
'   End If
   ...
   file.Write "public class " + FirstCharToU(pEntity.code) + "Controller {" + vbCrLf
...
'  End If
	...
	Dim attrHtml,attrColumn,attrColumnVal
    For Each attr In pEntity.Attributes
        If Not attr.IsShortcut Then  
...
Function AddMethod(pEntity)
	 ...
	 AddMethod = AddMethod  + "       "+FirstCharToU(pEntity.Code) +" "+FirstCharToL(pEntity.Code)+" = new "+FirstCharToU(pEntity.Code)+"();"+ vbCrLf
	 AddMethod = AddMethod  + "       "+ vbCrLf
	 AddMethod = AddMethod  + "       PageInfo<"+FirstCharToU(pEntity.Code)+"> pageInfo = this."+FirstCharToL(pEntity.Code)+"Service.queryPages("+FirstCharToL(pEntity.Code)+", new Integer(pageNum), new Integer(size));"+ vbCrLf
	 ...
	 AddMethod = AddMethod  + "		  "+FirstCharToU(pEntity.Code)+" new"+FirstCharToU(pEntity.Code)+" = "+FirstCharToL(pEntity.Code)+"Service.save("+FirstCharToL(pEntity.Code)+");" + vbCrLf
	 ...
	 AddMethod = AddMethod  + "           "+FirstCharToU(pEntity.Code)+" "+FirstCharToL(pEntity.Code)+" = this."+FirstCharToL(pEntity.Code)+"Service.searchById(id);" + vbCrLf
	 ...
	 AddMethod = AddMethod  + "	          String[] idArr = ids.split("","");" + vbCrLf
	 AddMethod = AddMethod  + "	          for (String id : idArr) {" + vbCrLf
	 AddMethod = AddMethod  + "	             this."+FirstCharToL(pEntity.Code)+ "Service.delete(id);" + vbCrLf
	 AddMethod = AddMethod  + "	          }" + vbCrLf
