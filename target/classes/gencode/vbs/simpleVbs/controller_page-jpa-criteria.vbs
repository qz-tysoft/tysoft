...
'  End If
	...
	Dim strList,attr
    Dim queryCodeHtml,modRow,countNum
    countNum = 0
        ...
        If Not attr.IsShortcut Then  
			If Not attr.PrimaryIdentifier  Then
			   ...
			   If GetType(attr.DataType) = "java.lang.String" Then
				...
				End If   
					queryCodeHtml = queryCodeHtml + "                        <label class=""control-label col-sm-1"" for=""query_title_like"">"+attr.Name + "</label>"+ vbCrLf
					queryCodeHtml = queryCodeHtml + "                        <div class=""col-sm-2""> <input type=""text"" class=""form-control"" id=""query_"+FirstCharToL(attr.Code)+"_like"" name=""query_"+FirstCharToL(attr.Code)+"_like"" /> </div>"+ vbCrLf
				If modRow =1 Then 
	...
	Dim attrHtmlHidden,attrHtml,attrColumn,attrColumnVal,attrColumnEmp
    For Each attr In pEntity.Attributes
        If Not attr.IsShortcut Then  
			If Not attr.PrimaryIdentifier And attr.Code <> "parentId"  Then
				...
				End If 
				attrHtml = attrHtml + "		            		<div class=""row"">" + vbCrLf
				attrHtml = attrHtml + "							    <div class=""col-md-offset-2 col-md-8 col-xs-8 col-sm-8"">" + vbCrLf
				attrHtml = attrHtml + "								    <div class=""form-group"">" + vbCrLf
				attrHtml = attrHtml + "									    <div class=""input-group"">" + vbCrLf
				attrHtml = attrHtml + "										  <span class=""input-group-addon"">"+ attr.name + "</span>"+ vbCrLf
				attrHtml = attrHtml + "										  <input type=""text"" class=""form-control update"" name="""+FirstCharToL(attr.Code)+""" id="""+FirstCharToL(attr.Code)+""" />"+ vbCrLf
			...
			End If
				attrColumnVal = attrColumnVal + "					$('#inputForm').find(""input[name='"+FirstCharToL(attr.Code)+"']"").val(json."+FirstCharToL(attr.Code)+");"+ vbCrLf
...
Function insertImportPage(file,pEntity)
	file.Write  vbCrLf
    file.Write   "import java.io.Serializable;"+vbCrLf
	file.Write   "import java.util.ArrayList;"+vbCrLf
	file.Write   "import java.util.HashMap;"+vbCrLf
	file.Write   "import java.util.Iterator;"+vbCrLf
...
'	file.Write   "import net.sf.json.JSON;"+vbCrLf
	...
	file.Write   "import com.xmrbi.iams.common.DateUtils;"+vbCrLf
...
'	 AddMethod = AddMethod  + "       if(StringUtils.isNotBlank(ordername)) {//排序-以""order_""开头可自动排序"+ vbCrLf
'	 AddMethod = AddMethod  + "           queryMap.put(""order_""+ordername, order);"+ vbCrLf
'	 AddMethod = AddMethod  + "       }"+ vbCrLf

'	 AddMethod = AddMethod  + "       PageInfo<"+FirstCharToU(pEntity.Code)+"> pageInfo = this."+FirstCharToL(pEntity.Code)+"Service.query"+FirstCharToU(pEntity.Code)+"PagesByMap(queryMap,search, new Integer(pageNum), new Integer(size));"+ vbCrLf
'	 AddMethod = AddMethod  + "       if(pageInfo.getTotal()>0){"+ vbCrLf
'	 AddMethod = AddMethod  + "           Map<String,Object> map = null;"+ vbCrLf
'	 AddMethod = AddMethod  + "           for("+FirstCharToU(pEntity.Code)+" "+FirstCharToL(pEntity.Code)+" : pageInfo.getList()){"+ vbCrLf
'	 AddMethod = AddMethod  + "               map = new HashMap<String,Object>();"+ vbCrLf
'	 For Each attr In pEntity.Attributes
'	     If Not attr.IsShortcut Then
'		    If GetType(attr.DataType)<> "java.util.Date" Then
'	 AddMethod = AddMethod  + "               map.put("""+FirstCharToL(attr.Code)+""", "+FirstCharToL(pEntity.Code)+".get"+FirstCharToU(attr.Code)+"());"+ vbCrLf
'	        Else
'     AddMethod = AddMethod  + "               map.put("""+FirstCharToL(attr.Code)+""", ("+FirstCharToL(pEntity.Code)+".get"+FirstCharToU(attr.Code)+"()"+"!=null) ? DateUtils.getTimeStr("+FirstCharToL(pEntity.Code)+".get"+FirstCharToU(attr.Code)+"()"+", DateUtils.HYPHEN_DISPLAY_DATE_MI) : """");"+ vbCrLf
'	        End If
'         End If
'    Next
...
'	 AddMethod = AddMethod  + "           }"+ vbCrLf
'	 AddMethod = AddMethod  + "       }"+ vbCrLf
'	 AddMethod = AddMethod  + "       resultMap.put(""page"", pageInfo.getPageNum());"+ vbCrLf
'	 AddMethod = AddMethod  + "       resultMap.put(""total"", pageInfo.getTotal());"+ vbCrLf
'	 AddMethod = AddMethod  + "       resultMap.put(""rows"", listMap);"+ vbCrLf
	 ...
	 AddMethod = AddMethod  + "//       Page<"+FirstCharToU(pEntity.Code)+"> pageInfo = this."+FirstCharToL(pEntity.Code)+"Service.queryPagesByMap(queryMap,searchText,pageable);"+ vbCrLf
	 AddMethod = AddMethod  + "       Page<"+FirstCharToU(pEntity.Code)+"> pageInfo = this."+FirstCharToL(pEntity.Code)+"Service.queryPages(searchText,pageable);"+ vbCrLf
	 AddMethod = AddMethod  + "       if(pageInfo.getSize()>0){"+ vbCrLf
	 AddMethod = AddMethod  + "           Map<String,Object> map = null;"+ vbCrLf
	 AddMethod = AddMethod  + "           for("+FirstCharToU(pEntity.Code)+" "+FirstCharToL(pEntity.Code)+" : pageInfo){"+ vbCrLf
	 AddMethod = AddMethod  + "               map = new HashMap<String,Object>();"+ vbCrLf
	 For Each attr In pEntity.Attributes
	     If Not attr.IsShortcut Then
		    If GetType(attr.DataType)<> "java.util.Date" Then
	 AddMethod = AddMethod  + "               map.put("""+FirstCharToL(attr.Code)+""", "+FirstCharToL(pEntity.Code)+".get"+FirstCharToU(attr.Code)+"());"+ vbCrLf
	        Else
     AddMethod = AddMethod  + "               map.put("""+FirstCharToL(attr.Code)+""", ("+FirstCharToL(pEntity.Code)+".get"+FirstCharToU(attr.Code)+"()"+"!=null) ? DateUtils.getTimeStr("+FirstCharToL(pEntity.Code)+".get"+FirstCharToU(attr.Code)+"()"+", DateUtils.HYPHEN_DISPLAY_DATE) : """");"+ vbCrLf
	        End If
         End If
     Next
	 AddMethod = AddMethod  + "               listMap.add(map);"+ vbCrLf
	 AddMethod = AddMethod  + "           }"+ vbCrLf
	 AddMethod = AddMethod  + "       }"+ vbCrLf
	 AddMethod = AddMethod  + "       resultMap.put(""page"", pageInfo.getTotalPages());"+ vbCrLf
	 AddMethod = AddMethod  + "       resultMap.put(""total"", pageInfo.getTotalElements());"+ vbCrLf
	 AddMethod = AddMethod  + "       resultMap.put(""rows"", listMap);"+ vbCrLf
	 ...
	 AddMethod = AddMethod  + "   @ResponseBody" + vbCrLf
	 AddMethod = AddMethod  + "   public String  find(HttpServletRequest request) {" + vbCrLf
	 AddMethod = AddMethod  + "       String id = request.getParameter(""id"");" + vbCrLf
	 ...
	 AddMethod = AddMethod  + "           if("+FirstCharToL(pEntity.Code)+" != null) {" + vbCrLf
	 AddMethod = AddMethod  + "               Map<String,Object> map = new HashMap<String,Object>();" + vbCrLf
     ...
     AddMethod = AddMethod  + "               map.put("""+FirstCharToL(attr.Code)+""", ("+FirstCharToL(pEntity.Code)+".get"+FirstCharToU(attr.Code)+"()"+"!=null) ? DateUtils.getTimeStr("+FirstCharToL(pEntity.Code)+".get"+FirstCharToU(attr.Code)+"()"+", DateUtils.HYPHEN_DISPLAY_DATE) : """");"+ vbCrLf
         ...
         End If
     Next
	 AddMethod = AddMethod  + "               JSONObject jo = JSONObject.fromObject(map);"+ vbCrLf
	 AddMethod = AddMethod  + "               return jo.toString();"+ vbCrLf
	 AddMethod = AddMethod  + "           }"+ vbCrLf
	 AddMethod = AddMethod  + "       }"+ vbCrLf
	 AddMethod = AddMethod  + "       return null;" + vbCrLf
	 AddMethod = AddMethod  + "   }" + vbCrLf
	 AddMethod = AddMethod  + "   " + vbCrLf
