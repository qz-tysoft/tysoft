...
var signerList=[];
var signerIds=[];
var beforeSignerList=$("#signerList").val();
var beforeSignerIds=$("#signerIds").val();
$(function () {
    //初始化div高度
    ...
    var singerTabletInit = new SingerTabletInit();
    singerTabletInit.Init();
    // //初始化角色表格
    ...
    signerList=[];
    signerIds=[];
    strs=beforeSignerList.split(","); //字符分割
    for (i=0;i<strs.length ;i++ )
    {
    	  if(strs[i].length>0){
    		  signerList.push(strs[i]);
          }
    } 
     
    strIds=beforeSignerIds.split(","); //字符分割
    for (i=0;i<strs.length ;i++ )
    {
      if(strIds[i].length>0){
    	  signerIds.push(strIds[i]);
      }
    } 
  ...
  //设置之前选中的人员
    if(signerList.length>0){
    	   for(var i=0;i<signerList.length;i++){
    	    	var signerName=signerList[i].split("-")[1];
    	    	var signerId=signerIds[i];
    	        addSignerSysuserShow(signerId,signerName);
    	    }
    }
            ...
            columns: [
                {
                    ...
                    if(signerIds.length>0){
                    	...
                    	if(flag==1){
                      		 return '<input type="checkbox" onclick="chooseCheckbox(this)"  checked=true  title='+r.name+'  value='+r.isValid+' id='+r.id+' name='+r.userDept+'   />';
                    	}else{
         					 return '<input type="checkbox" onclick="chooseCheckbox(this)"  title='+r.name+'  value='+r.isValid+' id='+r.id+' name='+r.userDept+'   />';
                    	}
...
//初始化会签人表格
var SingerTabletInit = function () {
    var singerTabletInit = new Object();
    //初始化Table
    singerTabletInit.Init = function () {
        $('#singerContent').bootstrapTable({
            url: CTX + 'organizationManage/query-systemuser-page',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            //toolbarAlign:'left',				//工具栏位置
            striped: true,                      //是否显示行间隔色
            ...
            sortable: true,                     //是否启用排序
            sortOrder: "asc",                   //排序方式
            queryParams: singerTabletInit.queryParams,//传递参数（*）
            sidePagination: "server",           //分页方式：client客户端分页，server服务端分页（*）
            pageNumber: 1,                       //初始化加载第一页，默认第一页
            pageSize: 10,                       //每页的记录行数（*）
            pageList: [10, 25, 50, 100],        //可供选择的每页的行数（*）
            ...
            columns: [
                {
                    ...
                    if(signerIds.length>0){
                    	...
                    	if(flag==1){
                      		 return '<input type="checkbox" onclick="editCheckbox(this)"  checked=true  title='+r.name+'  value='+r.isValid+' id='+r.id+' name='+r.userDept+'   />';
                    	}else{
         					 return '<input type="checkbox" onclick="editCheckbox(this)"  title='+r.name+'  value='+r.isValid+' id='+r.id+' name='+r.userDept+'   />';
                    	}
    ...
    //得到查询的参数
    singerTabletInit.queryParams = function (params) {
        var data = JSON.stringify($('#singerFormSearch').serializeJSON());
        var temp = {   //这里的键的名字和控制器的变量名必须一致，这边改动，控制器也需要改成一样的
            pageNum: params.pageNumber,//页码params.offset
            size: params.pageSize,//页面大小params.limit
            order: params.sortOrder,//排序 params.order
    ...
    return singerTabletInit;
};

//用户按照部门选人
function goSingerSysuser() {
    var sysuserId = $('#now_user_id').val();
    var lesseeId = $('#now_user_lesseeId').val();
    $("#singerContent").bootstrapTable('refresh');
    ...
    var r = Math.random().toString().substring(5);
    $.ajax({
        url: CTX + 'organizationManage/query-signer-sub-org-tree?Mathnum=' + r,
        method: 'GET',
        dataType: 'json',
...
function clickDept(node){
	$("#singerOrgId").val(node.id);
    $("#singerOrgName").html(node.text);
	$("#singerContent").bootstrapTable('refresh');
}

//添加到会签人表格
function addSignerSysuserShow(id, name) {
	//存在不给予添加
	if($("#edit__signer_id" + id + "").length!=0){
		return;
	}
	
    var htmlStr = '';
    htmlStr +=
        '<div id="edit__signer_id' + id + '" class="allRoleSysuser" style="background-color:#3daae8;color:white;width:auto;hight:60px;float:left;margin:5px;">' +
        '	<input type="hidden" name="sysuserid" class="sysuserid" value="' + id + '"/>' +
        '   <spand style="padding-left: 5px;padding-right: 5px;">' + name + '</spand>' +
        '   <input type="button" class="closeDiv" onclick="removeSigner(\'' + id + '\',0)" value="X" style="background:transparent;border:none;"/>' +
        '</div>';
    $("#edit_singer_name").append(htmlStr);
    
}
//删除
function removeSigner(id,type){
	//修改表格的复选框状态
	  var singer=$('#' + id).attr("name")+"-"+$('#' + id).attr("title");
      $('#' + id).attr('checked',false);
    if(type==0){
    	  signerList[signerList.indexOf(singer)] = null;
      	  signerList.splice( signerList.indexOf( null ), 1 );
      	  signerIds[ signerIds.indexOf(id)] = null;
      	  signerIds.splice( signerIds.indexOf( null ), 1 );
      }
      $('#edit__signer_id' + id).remove();
}


//合同id
function chooseCheckbox(node){
	var singer=node.name+"-"+node.title;
	  if ($(node).is(':checked')) {
		 signerList.push(singer);
		 var isContainId=signerIds.indexOf(node.id);
		 if(isContainId==-1){//0存在,-1不存在
			 signerIds.push(node.id); 
		 }
		 addSignerSysuserShow(node.id,node.title);
      }else if($(node).is(':checked')==false){
    	  signerList[ signerList.indexOf(singer)] = null;
    	  signerList.splice( signerList.indexOf( null ), 1 );
    	  signerIds[ signerIds.indexOf(node.id)] = null;
    	  signerIds.splice(signerIds.indexOf( null ), 1 );
    	  removeSigner(node.id,1);
      }  
}


//合同id-signer
function editCheckbox(node){
...
function sureView(){
	...
	var approveOpinion=$("#approveOpinion").val();
		$("#approveForm").attr("action",CTX+'contract-approve/upload-scan-contract-view?id='+id+"&taskId="+taskId+"&taskName="+taskName+"&signerList="+signerList+"&signerIds="+signerIds+"&approveOpinion="+approveOpinion);
		$("#approveForm").submit();
}
...
function closeView(){
	...
	var id=$("#contractId").val();
	var approveOpinion=$("#approveOpinion").val();
		$("#approveForm").attr("action",CTX+'contract-approve/upload-scan-contract-view?id='+id+"&taskId="+taskId+"&taskName="+taskName+"&approveOpinion="+approveOpinion);
		$("#approveForm").submit();
}
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            if (!json.success) {
                ...
                $('#orgId').val('');
                //新增、删除成员按钮恢复可用状态
                $("#btn_add_signer_member").attr('disabled',true);
                $('#orgTableContent').bootstrapTable('refresh');
                initOrgMenuTree();
                alert(json.msg);
            }
