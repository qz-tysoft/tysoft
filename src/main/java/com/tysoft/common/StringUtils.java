...
public class StringUtils {
     ...
     * @return
     */
    public static Double getDoubleFormat(Double db, String fmt) {
    	Double retu = 0d;
    	if (null == fmt || "".equals(fmt.trim())) {
    		return retu;
    	}
    	...
    	try {
    		DecimalFormat decimalformat = new DecimalFormat(fmt);
    		retu = getDouble(decimalformat.format(db));
    		decimalformat = null;
    	} catch (IllegalArgumentException iaex) {
    		// iaex.printStackTrace();
    	}
	 ...
	 * @return
	 */
	 public static String getStr(String sourceStr, int n,String str) {
	        int i = 0;
	        int s = 0;
	        while (i++ <= n) {
	            s = sourceStr.indexOf(str, s + 1);
	            if (s == -1) {
	                return sourceStr;
	            }
	        }
	        return sourceStr.substring(0, s);
	  }
    ...
    /**
     * 字串转成List（去除相同字符．如：＂a,b,c,c,d,d＂转成　＂[a,b,c,d]＂）
     * @param str1
     * @param sign1
     * @return
     */
    public static List<Object> strToList(String str1,String sign1){
    ...
    public static String changeCode(String old,int len){
        ...
        if(old.length()>len){
        	return old;
        	//throw new java.lang.ArrayIndexOutOfBoundsException("原字符串编号比生成的字符串长度长");
        }
        String frontStr = "";
