...
$.fn.addtabs = function (options) {
    ...
    Addtabs.options = $.extend({
        ...
        iframeHeight: $(document).height() - 65, //固定TAB中IFRAME高度,根据需要自己修改
        method: 'init',
...
/**
 * 子页面加载完毕时，处理iframe的样式
 */
function doFrameStyle(ifm) {
	var subWeb = ifm.contentWindow.document || ifm.document || ifm.contentDocument;
	var height = $(document).height() - 65;
	if(ifm != null && subWeb != null) {
		var tmpHeight = subWeb.body.scrollHeight;//自适应高度
//		ifm.height = height > tmpHeight ? height : tmpHeight;
	}
}