...
function paintCompList_body(comps){
	...
	var noFinanceComps = $("#noFinanceComps").val();
	var financeComps = $("#financeComps").val();
	var generalAssert = $("#generalAssert").val();
	var financeCount = 0;
	var generalAssertCount = 0;
	...
	for(var i=0,len=comps.length; i<len; i++){
		...
		if(comps[i].comp.indicator.fillDepartment != null && comps[i].comp.indicator.fillDepartment.name.indexOf('财务') > -1){
			if(comps[i].comp.indicator.indicName.indexOf("资产总额") > -1){
				if(generalAssertCount == 0){
				...
				if(financeCount == 0 && noFinanceComps == "yes"){
					var newRow = tBody.insertRow();
					var newCell0 = newRow.insertCell();
					newCell0.colSpan = 7;
					newCell0.innerHTML = "<span></span>";
					...
					var newCell23 = newRow2.insertCell();
					var newCell24 = newRow2.insertCell();
					var newCell25 = newRow2.insertCell();
					...
					newCell23.colSpan = '2';
					newCell24.align = "center";
					newCell25.align = "center";

					var year = $("#year").val();
					newCell20.innerHTML = "<span></span>";
					newCell21.innerHTML = "<span>单位</span>";
					newCell22.innerHTML = "<span>期初余额</span>";
					newCell23.innerHTML = "<span>本期收入</span>";
					newCell24.innerHTML = "<span>本期支出</span>";
					newCell25.innerHTML = "<span>期末余额</span>";
				}
			...
			//本月止年累
			if((comps[i].last_thisMonthAccumu != null && comps[i].comp.indicator.unit != '%') || readonly == "readonly" || comps[i].comp.indicator.indicNo.indexOf(".") < 0){
				newCell5.innerHTML = "<span id='thisMonthAccumu_"+ comps[i].comp.id +"'>" + num_thisMonthAccumu + "</span>";
			}else{
				newCell5.innerHTML = "<input class=\"form-control\" style=\"width: 120px;\" id=\"thisMonthAccumu_"+ comps[i].comp.id 
				+"\" value=\""+ num_thisMonthAccumu + "\" onchange=\"fnSetValue(\'"+comps[i].comp.indicator.unit+"\',this)\" />";
			}
			//计划完成率
			if(comps[i].comp.indicator.unit == '%' && readonly != "readonly"){
				newCell6.innerHTML = "<input class=\"form-control\" style=\"width: 120px;\" id=\"completionRate_"+ comps[i].comp.id 
...
function fnSetAccuAndRate(index){
	var plan = $('#thisYearPlan_'+ index).text();
	var value = $('#thisMonthAccumu_'+ index).text();
	if(value==null||value==''){
		value = $('#thisMonthAccumu_'+ index).val();
	}
	if(plan && plan>0 && value){
		plan = parseFloat(plan);
		value = parseFloat(value);
		var rate = value/plan;
		rate = rate*100;
		var rateStr = rate.toFixed(2)+"%";
		$('#completionRate_'+index).text(rateStr);
		$('#myform_comp_'+ index).find("input[name='completionRate']").val(rate.toFixed(2)+"%");
	}
}
...
function refreshParentValue(comp){
	if(comp.indicator.indicNo.indexOf(".") > -1){//非顶级指标
		if(comp.indicator.fillDepartment == null || comp.indicator.fillDepartment.name.indexOf('财务') < 0){
			$("#lastYearAccumu_" + comp.id).text(comp.lastYearAccumu);
			$("#thisYearPlan_" + comp.id).text(comp.thisYearPlan);
			$("#thisMonthAmount_" + comp.id).val(comp.thisMonthAmount);
			$("#thisMonthAccumu_" + comp.id).text(comp.thisMonthAccumu);
			$("#thisMonthAccumu_" + comp.id).val(comp.thisMonthAccumu);
			$("#completionRate_" + comp.id).text(comp.completionRate);
		}else{
